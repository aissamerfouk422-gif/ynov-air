...
from .models import Flight, Airport, Passenger, Booking, Meal
import random
import string
...
def booking_create(request, flight_id):
    ...
    if request.method == 'POST':
        ...
        if flight.available_seats >= number_of_passengers:
            meal_price_total = (meal_obj.price if meal_obj else 0) * number_of_passengers
            booking = Booking.objects.create(
                booking_reference=booking_reference,
                flight=flight,
                passenger=passenger,
                number_of_passengers=number_of_passengers,
                total_price=flight.price * number_of_passengers + meal_price_total,
                status='CONFIRMED',
                meal=meal_obj,
                description=(meal_obj.description if meal_obj else '')
            )
    ...
    context = {'flight': flight, 'meals': meals}
